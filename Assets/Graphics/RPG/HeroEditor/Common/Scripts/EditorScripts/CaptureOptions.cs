...
namespace Assets.HeroEditor.Common.Scripts.EditorScripts
{
    public class CaptureOptions : MonoBehaviour
    {
        ...
        public void Capture()
        {
            ...
            if (Shot.isOn)
            {
                var character = UnityEngine.Object.FindFirstObjectByType<Character>();
            ...
            UnityEngine.Object.FindFirstObjectByType<SpriteSheetCapture>().Capture(options, int.Parse(FrameSize.text), int.Parse(FrameCount.text), Shadow.isOn);
            Close();
        }
